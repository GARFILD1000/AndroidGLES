...
class Sphere(radius: Float, n: Int): Shape() {
    ...
    init {
        ...
        vertexBuffer = initBuffer(vertices.toFloatArray())
        colorBuffer = initBuffer(colors.toFloatArray())
        indexBuffer = ByteBuffer.allocateDirect(indices.map(Int::toShort).toShortArray().size * 2)
            .apply {
                order(ByteOrder.nativeOrder())
            }.asShortBuffer().apply {
                put(indices.map(Int::toShort).toShortArray())
                position(0)
            }
